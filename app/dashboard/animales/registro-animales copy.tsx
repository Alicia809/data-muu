...
export default function AnimalesPage() {
  ...
  const fetchTipo_Semoviente = async () => {
    ...
    setError("");
    try {
      const res = await fetch("/api/tipo_animal");
      const data = await res.json();
  ...
  // Datos de ejemplo
  const [tipo_animal, setTipoSemoviente] = useState<Tipo_Semoviente[]>([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await res.json();
      fetchSemoviente(); // recarga lista desde la API
      setFormData({
        tipo_animal: "",
        color: "",
        venteado: "",
  ...
  const handleSubmit2 = async (e: React.FormEvent) => {
    ...
    try {
      let res;      
      res = await fetch("/api/tipo_animal", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      await res.json();
      fetchTipo_Semoviente(); // recarga lista desde la API
      setFormData2({
        nombre: "",
        descripcion: "",
      });
  ...
  const handleDelete2 = async (id: number) => {
    if (confirm("¿Está seguro de que desea eliminar este semoviente?")) {
      try {
        await fetch(`/api/tipo_animal?id=${id}`, { method: "DELETE" });
        setSuccess("Semoviente eliminado correctamente");
        // Limpiar la alerta de éxito después de 10 segundos
        setTimeout(() => setSuccess(""), 10000);
        fetchTipo_Semoviente(); // recarga lista desde la API
  ...
  const filteredAnimales = animales.filter(
    (animal) =>
      ...
      animal.venteado.toLowerCase().includes(searchTerm.toLowerCase()) 
  )
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="parrafo text-muted-foreground">
                Gestione el registro de semovientes 
              </p>
            </div>
          </div>
